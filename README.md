...
hostsに以下を追加します。
これによりブラウザから本アプリケーションのWebページにアクセスできるようになります。

```
127.0.0.1	admin.t.isucon.dev
127.0.0.1	t.isucon.dev
```

実際は証明書の有効期限が切れているため、ブラウザの保護機能によりアクセスすることができません。
Chromeを使用している場合は、証明書の警告が出ているページで `thisisunsafe` と入力することでアクセスできます。

またリポジトリルートで以下のMakeタスクを実行します。
